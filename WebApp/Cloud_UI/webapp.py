...
def display_heatmap():
  ...
  heatmap = px.density_heatmap(data_value, x="polarity", y="subjectivity", template='plotly_dark')
...
def display_barplot():
  ...
  plot.update_layout(legend_title_text='Legend', template='plotly_dark')
...
def display_line_graphs():
  ...
  print([x, y1, y2])
  dic = {'regions':x,'polarity':y1,'low_income_value':y2}
...
def display_piechart():
  ...
  color= ['red', 'blue']  
  pie = generate_pie_chart(df)
  pie_chart = px.pie(values=pie, names=names, color=names,color_discrete_map={names[0]: color[0], names[1]: color[1]},template='plotly_dark')
  pieJSON = json.dumps(pie_chart, cls= plotly.utils.PlotlyJSONEncoder)
  return render_template('Fourth_scenario.html', pieJSON = pieJSON) 
...
def display_twin_barplots():
  data, x, y = twin_barplots(df)
  x_axis = np.arange(len(x))

  plot = go.Figure()
  plot.add_trace(go.Bar(x=x,y=y,name="Polarity"))
  plot.add_trace(go.Bar(x=x, y=[1,2,3,4],name="Number of GP's per thousand people"))  
  plot.update_traces(texttemplate='%{text:.2s}')
  plot.update_layout(legend_title_text='Legend', template='plotly_dark')
  plot.update_yaxes(title_text="<b>Polarity</b>", autorange=True)
